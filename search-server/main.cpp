...
public:
    ...
    void AddDocument(int document_id, const string& document, DocumentStatus status,
        ...
        if (document_id < 0 || documents_.count(document_id)) {
            throw invalid_argument("Bad document id " + document_id);
        }
    ...
    int GetDocumentId (int doc_index) {
        return (documents_ID.at(doc_index));
    }
private:
    struct DocumentData {
        int rating;
    ...
    vector<string> SplitIntoWordsNoStop(const string& text) const {
        ...
        for (const string& word : SplitIntoWords(text)) {
            if (!IsValidWord(word)) {
                throw invalid_argument("Bad word "s + word);
            }
            if (!IsStopWord(word)) {
                words.push_back(word);
            }
    ...
    void ParseQueryWord(string text, QueryWord& word) const {
        bool is_minus = false;
        if (!IsValidWord(text)) {
            throw invalid_argument("Bad text ParseQueryWord");
        }
        ...
        if (text[0] == '-') {
            ...
            if (text.size() == 0) {
             ...
             if (text[0] == '-') {
                throw invalid_argument("hyphen after hyphen");
            }
        }
